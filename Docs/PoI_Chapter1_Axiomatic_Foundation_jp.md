...
*   **質量（Structural Mass/Inertial）**: 物理的質量ではなく、並行鍵 $K$ の更新に対する抵抗、あるいは背景幾何 $R$ との結合強度 $\gamma$ に由来する「構造的慣性」を指す。
*   **ゲージ対称性（Gauge Symmetry）**: 知能内部の表現形式の任意性（自由度）を指し、その「破れ」は特定の論理的決定や構造の固定化（不連続な相転移）に対応する。
*   **相転移（Phase Transition）**: 秩序変数の非連続的な変化を伴う状態遷移を指し、知能においては有効次元 $d_{\text{eff}}$ の跳躍として観測される。
